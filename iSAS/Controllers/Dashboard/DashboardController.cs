...
namespace ISas.Web.Controllers.Dashboard
{
    ...
    public class DashboardController : Controller
    {
        ...
        public ActionResult Dashboard()
        {
            ...
            {
                Dashboard_StudentModel studModel = _dashRepo.GetDashBoard_StudentDetails(Session["UserID"].ToString(), Roles.GetRolesForUser(User.Identity.Name).FirstOrDefault(), DateTime.Now.ToShortDateString().Replace("-", "/"), Session["SessionId"].ToString());
            ...
            {
                return View("Dashboard_Admin", _dashRepo.GetAdminDashboardDetails(DateTime.Now.ToShortDateString(), Session["SessionID"].ToString(), Session["UserID"].ToString()));
            }
            else if (User.IsInRole("Principal") || User.IsInRole("Coordinator") || User.IsInRole("Class Teacher")
                || User.IsInRole("User"))
            {

                //ViewBag.staffId = _staffRepo.GetStaffIDByUserID(Session["UserID"].ToString());
                return View("Dashboard_Staff", _dashRepo.GetDashBoard_StaffDetails(Session["UserID"].ToString(), Session["SessionID"].ToString()));
            }
            return View();
        }
        ...
        public JsonResult GetAttenSummery()
        {
            DashboardModel_Admin model = _dashRepo.GetAdminDashboardDetails_Atten(DateTime.Now.ToShortDateString(), Session["SessionID"].ToString());
            var result = model.AttnStatusList.Select(r => new
            {
                AttenStatus = r.AttenStatus,
                NoOfStudent = r.NoofCount
            }).ToArray();
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetStudentStrength()
        {
            DataSet ds = _dashRepo.GetAdminDashboardDetails_Strength(DateTime.Now.ToShortDateString(), Session["SessionID"].ToString());
            DashboardModel_Admin model = new DashboardModel_Admin();
            for (int i = 2; i < ds.Tables[3].Columns.Count; i++)
            {
                model.SectionList.Add(ds.Tables[3].Columns[i].ColumnName);
            }
            var array = DataTableX.ToDynamic(ds.Tables[3]).ToArray();

            int totalStrength = 0;
            for (int i = 0; i < ds.Tables[3].Rows.Count; i++)
            {
                for (int j = 2; j < ds.Tables[3].Columns.Count; j++)
                {
                    int classStrength = 0;
                    int.TryParse(ds.Tables[3].Rows[i][j].ToString(), out classStrength);
            ...
            return Json(new { Strength = json, Sections = sectionList, TotalStrength = totalStrength }, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetStaffAttendance()
        {
            DataSet ds = _dashRepo.GetAdminDashboardDetails_StaffAtten(DateTime.Now.ToShortDateString(), Session["SessionID"].ToString());

            var result = ds.Tables[5].AsEnumerable().Select(r => new
            {
        ...
        public ActionResult StudentAttendanceDetails()
        {
            ...
            model.AttenDetails = _dashRepo.AttendanceDetails(Session["UserId"].ToString() , Session["SessionId"].ToString());
            model.AttDefaulterMessage = _dashRepo.GetAttendanceDefaulterMessage(Session["UserId"].ToString(), Session["SessionId"].ToString());
        ...
        {
            List<AttnDetailsModel> attenDetails = _dashRepo.GetStudentAttenDetails_BySession(sessionId, Session["UserId"].ToString());
        ...
        {
            List<Tuple<string, string>> attendenceDetails = this._dashRepo.GetStudentAttenDetails(month, year, Session["UserId"].ToString());
            WeekForMonth model = getCalender(month, year, attendenceDetails, Session["UserId"].ToString());
