...
namespace ISas.Web.Controllers
{
    ...
    public class Student_RegistrationController : Controller
    {
        ...
        public Student_RegistrationController(IStudentClass studentClass, IStudentSession studentSession, ICountryRepository country, IAreaRepository area, ISchool_DocumentMasterRepo schoolDoc, IStudentRegistrationRepo studentReg, ISchoolAdmissionCategoryRepo admCategory, IStateRepository states, ICityRepository cities)
        {
        ...
        //public PartialViewResult _Student_Registration_PrintPage()
        //{
        //    return PartialView(_studentRegRepo.Student_Registration_MasterDetails("").ToList());
        //}
        ...
        //    DataTable dt = _studentRegRepo.ValidateDOB(classId);
        //    //int upperAge = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["UpperAge"]);
        //    //int lowerAge = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["LowerAge"]);
        //    decimal upperAge = Convert.ToDecimal(dt.Rows[0][1]);
        //    decimal lowerAge = Convert.ToDecimal(dt.Rows[0][0]);
        //    DateTime studentDOB = new DateTime();
        //    try
        //    {
        //        if (DateTime.TryParse(DOB, out studentDOB))
        //        {
        //            int years = new DateTime(dateAsOn.Subtract(studentDOB).Ticks).Year - 1;
        //            DateTime pastYearDate = studentDOB.AddYears(years);
        //            int months = 0;
        //            for (int i = 1; i <= 12; i++)
        //            {
        //                if (pastYearDate.AddMonths(i) == dateAsOn)
        //                {
        //                    months = i;
        //                    break;
        //                }
        //                else if (pastYearDate.AddMonths(i) >= dateAsOn)
        //                {
        //                    months = i - 1;
        //                    break;
        //                }
        //            }
        //            int days = dateAsOn.Subtract(pastYearDate.AddMonths(months)).Days;
        //            if (years >= lowerAge && years <= upperAge)
        //                return new Tuple<bool, string>(true, "success");
        //            return new Tuple<bool, string>(false, string.Format("Age Is {0} Y , {1} M , {2} D. And it should between {3} Y to {4} Y as on {5}", years, months, days, lowerAge, upperAge, dateAsOn));
        //        }
        //    }
        //    catch
        //    {
